...
class series_decomp(nn.Module):
    """
    Series decomposition block using RecurrentCycle
    """
    ...
    def __init__(self, cycle_len, channel_size):
        ...
        self.cycle_len = cycle_len
        self.channel_size = channel_size
        self.recurrent_cycle = RecurrentCycle(cycle_len, channel_size)

    def forward(self, x, index):
        """
        x: 输入时间序列数据 [Batch, Input Length, Channel]
        index: 起始索引，用于周期性采样
        """
        batch_size, input_len, channel = x.shape

        # 提取趋势分量
        trend_init = self.recurrent_cycle(index, input_len)  # [Batch, Input Length, Channel]

        # trend_init = trend_init.unsqueeze(0).repeat(batch_size, 1, 1)  # 扩展到 batch 维度

        # 计算残差分量
        res_init = x - trend_init

        return res_init, trend_init
